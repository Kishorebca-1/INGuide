...
const loginPanel =
  document.querySelector(".login-panel:not(.forgot-panel)");
...
const forgotStatusMessage =
  document.getElementById("forgot-status-message");
    ...
    async (event) => {
      ...
      try {
        ...
        if (forgotStatusMessage) {
          ...
          forgotStatusMessage.style.color =
            "#e5ffef";

          forgotStatusMessage.classList.add(
            "is-visible"
          );
        }
...
const googleModal =
  document.getElementById("google-modal");

const googleClose =
  document.getElementById("google-close");

const googleOverlay =
  document.getElementById("google-overlay");

const googleAccount =
  document.getElementById("google-account");

const anotherAccount =
  document.getElementById("another-account");


// Open Google modal
if (false && googleSignInButton && googleModal) {
...
if (googleOverlay) {
  googleOverlay.addEventListener(
    "click",
    closeGoogleModal
  );
}
...
// Real Google OAuth is NOT connected yet.
if (googleAccount) {
  googleAccount.addEventListener(
    "click",
    () => {
      closeGoogleModal();
...
if (anotherAccount) {
  anotherAccount.addEventListener(
    "click",
    () => {
      showStatus(
        "Real Google authentication is not connected yet.",
        "error"
      );
    }
  );
...
if (false && registerGoogleButton) {
  registerGoogleButton.addEventListener(
    "click",
    () => {
      alert(
        "Google authentication is not connected yet."
...
function initializeGoogleSignIn(attempt = 0) {
  if (!window.google || !window.google.accounts || !window.WELLNESS_GOOGLE_CLIENT_ID) {
    if (attempt < 20) {
...
const resetStatusMessage =
  document.getElementById("reset-status-message");
...
if (
  ...
  if (!resetToken) {
    ...
    resetStatusMessage.style.color =
      "#ffd0cc";

    resetStatusMessage.classList.add(
      "is-visible"
    );
